...
### Patients 
The patient stores all the information entered from the tirage form and used in the admin waitlist.
Each record represents a single triage submission for one patient, injury type, pain level ,and assigned priority level.

### Priorities 
The priority entities define the different urgency levels used in a system (for example the red/yellow/green states visble in the admin UI).
It stores a human-readable description, a color used in the interface, and an estimated wait tiime for each level.

### Action_Logs 
The action entity records every action that an admin performs on a patient from the admin dashboard.
It allows the system to keep the history of changes (eg a patient's attention level has been increased, a patient being removed from the queue).
...
- `injury_type` (varvhar): The region of the body or the injury type selected from the form (e.g., “Head”, “Chest”, “Abdomen”, “Neck”, “Back”, “Arm or Leg”).
...
- `color_code` (varchar): Hex color used in the UI to represent this level  
  (e.g., green for stable patients, yellow for caution, red for critical).
